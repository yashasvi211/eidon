...
export default function DeepStats({ tasks }: DeepStatsProps) {
  ...
  const stats = useMemo(() => {
    ...
    let totalWorked = 0;
    let totalWindows = 0;
    let taskBreakdown: { title: string; worked: number; alloc: number; windows: number; project: string }[] = [];
    let groupedTime: { [key: string]: { worked: number; windows: number; date: number } } = {};
      ...
      <View style={[styles.section, { backgroundColor: colors.ghSurface, borderColor: colors.ghBorder }]}>
        ...
        {stats.timeline.length === 0 ? (
          ...
          <View style={styles.chartContainer}>
            {stats.timeline.slice(0, 10).map((item) => (
              <View key={item.key} style={styles.chartRow}>
                <Text style={[styles.chartRowLabel, { color: colors.ghText }]}>{item.key}</Text>
              ...
              return (
                <View key={idx} style={[styles.breakdownItem, { borderBottomColor: colors.ghBorder }]}>
                  ...
                  <View style={styles.breakdownMeta}>
                    <Text style={[styles.breakdownMetaText, { color: colors.ghMuted }]}>
